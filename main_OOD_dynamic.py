...
from WOOD_Loss import sink_dist_test, NLLWOOD_Loss
    ...
    #for InD_images, InD_labels in InD_train_loader:
        ...
        if not (count % 400):    # It's same as "if count % 100 == 0"
            ...
            for images, labels in InD_test_loader:
                ...
                InD_sink_dist = sink_dist_test(outputs, labels, num_class[0], device).cpu().detach().numpy()
            ...
            for images, labels in OOD_test_loader:
                ...
                outputs = model(test)
                total += len(labels)
                OOD_sink_dist = sink_dist_test(outputs, labels, num_class[0], device).cpu().detach().numpy()
